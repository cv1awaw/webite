module.exports = (req, res) => {
    ...
    <body>
        <div style="display: flex; justify-content: space-between; align-items: center;">
            ...
            <a href="/api/admin?key=${key}&action=test" style="padding: 10px 20px; background-color: #0070f3; color: white; text-decoration: none; border-radius: 4px;">Generate Test Log</a>
        </div>
        <p>Total Logs: ${logs.length} (Logs reset on deployment/restart)</p>
        <table>
            <tr>
                <th>Timestamp</th>
                <th>User Email</th>
                ...
                <th>Department</th>
                <th>Status</th>
                <th>Details (Steps & Error)</th>
            </tr>
    `;

    logs.forEach(log => {
        const statusClass = log.status === 'Success' ? 'success' : 'failed';
        const stepsLog = log.steps ? log.steps.join('<br>') : 'No steps recorded';
        const errorDetails = log.errorDetails ? `<details><summary>Stack Trace</summary><pre>${log.errorDetails}</pre></details>` : '';
            ...
            <tr>
                ...
                <td>
                    <details>
                        ...
                        <div style="font-size: 12px; background: #f9f9f9; padding: 5px;">${stepsLog}</div>
                        ${errorDetails}
                    </details>
                </td>
            </tr>
